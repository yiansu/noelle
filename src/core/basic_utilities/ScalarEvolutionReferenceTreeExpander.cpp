 ...
 * in an attempt to NOT expand values inline but rather at some arbitrary
 * insertion block
 */
...
ReferenceTreeExpander::ReferenceTreeExpander(
  ...
  /*
   * Visit every node in the tree, expanding non-referenced values using the
   * IRBuilder given
   */
  rootValue = visit(tree->getSCEV());

  /*
   * The current node acts like the top of a stack so must be set back after a
   * call to visit
   */
  currentNode = tree;
}
...
std::pair<Value *, Value *> ReferenceTreeExpander::visitTwoOperands(
  ...
  if (currentNode->getNumChildReferences() < 2) {
    // currentNode->getSCEV()->print(errs() << "ERROR: Failed to expand: ");
  ...
  if (!LHS || !RHS) {
    // TODO: Properly handle deleting values that are created by this class and
    // not SE scevs if (LHS) LHS->deleteValue(); if (RHS) RHS->deleteValue();
    return std::make_pair(nullptr, nullptr);
  }
...
Value *ReferenceTreeExpander::visitAddRecExpr(const SCEVAddRecExpr *S) {
  ...
  //   && "Expanding add recursive SCEV not implemented; it must be directly
  //   referenced");
  // return currentNode->getValue();
}
...
Value *ReferenceTreeExpander::visitUnknown(const SCEVUnknown *S) {
  // assert(valuesToReferenceAndNotExpand.find(currentNode->getValue()) !=
  // valuesToReferenceAndNotExpand.end()
  // && "Non SCEV value that cannot be expanded is not expected to be referenced
  // either");
  auto unkValue = currentNode->getValue();
  if (valuesToReferenceAndNotExpand.find(unkValue)
      != valuesToReferenceAndNotExpand.end()) {
    return currentNode->getValue();
  }
  return nullptr;
